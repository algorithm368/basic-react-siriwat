...
function ThemedButton() {
  ...
  return (
    <div className={`min-h-screen font-mono transition-all duration-500 ${bgClass} ${themeClasses}`}>
      ...
      <div className="relative z-10 max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="mb-12 text-center">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center space-x-4">
              <div className={`text-sm font-bold px-3 py-1 rounded-full ${darkMode ? 'bg-gray-800 text-green-400' : 'bg-green-100 text-green-800'}`}>
          ...
          <div className="relative">
            <h1 className={`text-4xl sm:text-5xl lg:text-6xl font-bold mb-4 ${textClass} transform transition-all duration-1000`}>
              <span className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">
                📌 React
              </span>
              <br />
              <span className="text-2xl sm:text-3xl lg:text-4xl">พื้นฐานที่ควรรู้</span>
            </h1>
        ...
        <div className="grid gap-6 mb-12">
          {sections.map((section, index) => {
            ...
            return (
              ...
              >
                <div className={`group relative overflow-hidden rounded-2xl border-2 transition-all duration-500 hover:scale-[1.02] ${
                  darkMode 
                  ...
                  {/* Gradient overlay */}
                  <div className={`absolute inset-0 bg-gradient-to-r ${section.color} opacity-0 group-hover:opacity-5 transition-opacity duration-500`} />
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <div className={`p-3 rounded-xl bg-gradient-to-r ${section.color} text-white transform group-hover:scale-110 transition-transform duration-300`}>
                          <Icon size={24} />
                        </div>
                        <div>
                          <h2 className={`text-lg sm:text-xl font-bold ${textClass} group-hover:text-blue-600 transition-colors duration-300`}>
                            {section.title}
                          </h2>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div className={`transform transition-transform duration-300 ${isActive ? 'rotate-90' : ''} ${textClass}`}>
                  ...
                  {isActive && (
                    <div className="px-6 pb-6 animate-fade-in">
                      <div className={`h-px bg-gradient-to-r ${section.color} mb-6 opacity-30`} />
                      
                      <div className="space-y-6">
                        <div className={`${darkMode ? 'text-gray-300' : 'text-gray-700'} whitespace-pre-line leading-relaxed text-base`}>
                          {section.content}
                        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className={`${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
                <tr>
                  <th className={`text-left py-4 px-6 font-bold ${textClass}`}>หัวข้อ</th>
                  <th className={`text-left py-4 px-6 font-bold ${textClass}`}>หน้าที่</th>
                </tr>
              </thead>
              <tbody>
                {summaryTable.map(([topic, description], index) => (
                  <tr key={index} className={`border-b transition-colors duration-200 ${
                    ...
                    <td className={`py-4 px-6 font-semibold ${textClass}`}>{topic}</td>
                    <td className={`py-4 px-6 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{description}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="mt-12 text-center">
          <div className={`inline-block p-6 rounded-2xl ${
            darkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white/80 backdrop-blur-sm border border-white/50 shadow-xl'
          }`}>
            <p className={`${darkMode ? 'text-gray-300' : 'text-gray-600'} text-lg`}>
              🚀 ถ้าต้องการเพิ่มเติม: 
              <span className="font-bold text-blue-600"> Routing, Form, API, Custom Hook </span>
              หรือ <span className="font-bold text-purple-600">Mindmap</span> สามารถแจ้งมาได้เลย!
            </p>
